            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        ...
                        <h5 class="float-right">Tambah Pengantaran</h5>
                    </div>
                    <form id="pengantaranform" style="display: none;">
                        <input type="hidden" id="user_id" name="user_id" value="" readonly>
                        <div class="card-body">
                            <div class="form-group">
                                <label for="pgnid">Pengantaran ID</label>
                                <input type="text" class="form-control" id="pgnid"
                                    placeholder="Masukkan pengantaran">
                            </div>
                            <div class="form-group">
    ...
    <script>
        ...
        $(document).ready(function() {
            var table = $('#example1').DataTable({
                "ajax": "{{ route('pengantaran.get') }}",
                "columns": [{
                        "data": "pengantaran_id",
                        "title": "<div style='text-align:center;'>Pengantaran ID</div>",
                    },
                    {
                        "data": "nama",
        ...
        // Start Hapus-Button
          $(document).ready(function() {
            $('#example1').on('click', '.hapus-button', function() {
                var pengantaranId = $(this).data('user-id');
                console.log('Tombol "Hapus" diklik. pengantaranId:', pengantaranId);
                Swal.fire({
                    title: 'Konfirmasi',
                    text: 'Apakah Anda yakin ingin menghapus pengantaran ini?',
                    icon: 'warning',
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: "{{ route('pengantaran.destroy', ['id' => ':pengantaranId']) }}".replace(
        ...
        $(document).ready(function() {
            ...
            $('#example1').on('click', '.ubah-button', function() {
                ...
                // Lakukan permintaan AJAX untuk mendapatkan data pengguna berdasarkan pengantaranId
                $.ajax({
                    url: "{{ route('pengantaran.show', ':pengantaranId') }}".replace(':pengantaranId', pengantaranId),
                    type: 'GET',
                    success: function(response) {
                        // Isi nilai-nilai form dengan data pengguna yang diterima
                        $('#user_id').val(response.id); // Set user_id untuk pembaruan data
                        $('#pgnid').val(response.pengantaran_id);
            ...
            $("#nm, #rlid, #alm, #ntlp").keydown(function(event) {
            ...
            function simpanData() {
                ...
                $.ajax({
                    url: "{{ route('pengantaran.simpan') }}", // Rute untuk menyimpan atau mengupdate data
                    type: 'POST',
                    data: {
                        "_token": "{{ csrf_token() }}",
                        "user_id": user_id,
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            const formContainer = document.getElementById("pengantaranform");
